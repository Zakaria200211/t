...
import time , json , string , os , re , random , uuid , subprocess , requests , sys , telebot ; from telebot import types ; from os import system
              ########## - Starting Code - #########
sTo = telebot.TeleBot(input("- Enter Token : "))
@sTo.message_handler(commands=['start'])
def Start(message):
	id_tele = message.from_user.id
	get_list = types.InlineKeyboardButton(text='- Get List .',callback_data='get_list')
	Add_Cookie = types.InlineKeyboardButton(text='- Add Account .',callback_data='Add_Cookie')
	Ron = types.InlineKeyboardMarkup(row_width=2);Ron.add(get_list,Add_Cookie)
	sTo.send_message(message.chat.id,text='- اهلاً بك عزيزي ، اختر القسم الذي يناسبك من الأزرار الآتيه :',reply_markup=Ron)
...
def Get_List(message):
	...
	sTo.send_document(message.chat.id,open(f'Id-{message.from_user.id}.txt', 'rb'),caption='- Total Id To Extract [ {} ] .'.format(idss))
